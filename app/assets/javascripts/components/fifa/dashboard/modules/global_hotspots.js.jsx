var GlobalHotspots = React.createClass({
  ...
  createMap: function() {
    ...
    var clusterSQL = $('#global_hotspots_sql').html();
    ...
    var sql = new cartodb.SQL({ user: 'amandacoston' });

    sql.execute(clusterSQL)
      .done(function(data) {
  ...
  render: function() {
    ...
    return (
      <div id="global_hotspots" className="dashboard-module" style={hiddenStyle}>
        ...
        <script type="sql/html" id="global_hotspots_sql">
          WITH
              hgridA AS (
                  SELECT CDB_HexagonGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * 48),
                  greatest(!pixel_width!,!pixel_height!) * 48) as cell),
              ...
              bigs AS (
                  SELECT * FROM (SELECT ST_Centroid(ST_Collect(i.the_geom_webmercator)) as the_geom_webmercator,
                      ...
                      avg(sentiment_score) AS average FROM hgridA,
                      (select * from sojcp0flmlm7rshw6usbx27) i where ST_Intersects(i.the_geom_webmercator, hgridA.cell)
                          GROUP BY hgridA.cell) t WHERE points_count > 100 ),

              hgridB AS (
                  SELECT CDB_HexagonGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * 36),
                      greatest(!pixel_width!,!pixel_height!) * 36) as cell),

              mids AS (
                  SELECT * FROM (SELECT ST_Centroid(ST_Collect(i.the_geom_webmercator)) as the_geom_webmercator,
                      count(i.cartodb_id) as points_count, 1 as cartodb_id, array_agg(cartodb_id) AS id_list,
                      avg(sentiment_score) AS average FROM hgridB,
                      (select * from sojcp0flmlm7rshw6usbx27) i where ST_Intersects(i.the_geom_webmercator, hgridB.cell) AND cartodb_id NOT IN (SELECT unnest(id_list) FROM bigs)
                          GROUP BY hgridB.cell) t WHERE points_count > 25 ),

              hgridC AS (
                  SELECT CDB_HexagonGrid(ST_Expand(!bbox!, greatest(!pixel_width!,!pixel_height!) * 12),
                      greatest(!pixel_width!,!pixel_height!) * 12) as cell),

              smalls AS (
                  SELECT * FROM (
                      SELECT ST_Centroid(ST_Collect(i.the_geom_webmercator)) as the_geom_webmercator,
                      count(i.cartodb_id) as points_count, 1 as cartodb_id, array_agg(cartodb_id) AS id_list,
                      avg(sentiment_score) AS average FROM hgridC,
                      (select * from sojcp0flmlm7rshw6usbx27) i where ST_Intersects(i.the_geom_webmercator, hgridC.cell) AND cartodb_id NOT IN (SELECT unnest(id_list) FROM bigs) AND cartodb_id NOT IN (SELECT unnest(id_list) FROM mids)
                          GROUP BY hgridC.cell) t WHERE points_count > 1 )

          SELECT the_geom_webmercator, 1 points_count, cartodb_id, ARRAY[cartodb_id], sentiment_score AS average, 'origin' as src FROM sojcp0flmlm7rshw6usbx27 WHERE cartodb_id NOT IN (select unnest(id_list) FROM bigs) AND cartodb_id NOT IN (select unnest(id_list) FROM mids) AND cartodb_id NOT IN (select unnest(id_list) FROM smalls)
