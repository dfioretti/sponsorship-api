      ...
      if (this.options.display) {
        ...
        if (this.isHorizontal()) {
          ...
          helpers.each(this.ticks, function(label, index) {
            // Blank ticks
            if (index % 2 == 1 || index + 1 >= this.ticks.length) {
              return;
            }

            var x1 = this.getPixelForTick(index);
            var x2 = this.getPixelForTick(index + 1);
            ...
            if (this.options.gridLines.display) {
              ...
              //   this.ctx.fillStyle = this.options.gridLines.color;
              //   setContextLineSettings = true; // reset next time
              if (setContextLineSettings) {
              ...
              // Draw the fill
              if (this.options.gridLines.drawOnChartArea) {
                this.ctx.moveTo(x1, chartArea.top);
                this.ctx.lineTo(x1, chartArea.bottom);
                this.ctx.lineTo(x2, chartArea.bottom);
                this.ctx.lineTo(x2, chartArea.top);
                this.ctx.closePath();
                this.ctx.fill();
              }
  ...
  Chart.Tooltip = Chart.Element.extend({
    ...
    draw: function() {
      ...
      if (this._options.tooltips.enabled) {
        ...
        ctx.strokeStyle = helpers.color('#e7e7e7').alpha(opacity).rgbString();
      ...
      if (this._options.tooltips.enabled) {
        ctx.fillStyle = helpers.color(vm.backgroundColor).alpha(opacity).rgbString();
        ctx.strokeStyle = helpers.color('#fff').alpha(opacity).rgbString();
